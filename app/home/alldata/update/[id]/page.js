...
export default function UpdateEquipmentPage({ params }) {
  ...
  if (loading) {
    return (
      <div className="page-wrapper" style={{ maxWidth:640, margin:'0 auto', textAlign:'center', paddingTop:60 }}>
      ...
      <div className="form-card-body">
        {status === 'success' && (
          <div className="alert-success" style={{ marginBottom:20 }}>
            ✅ อัปเดตสำเร็จแล้ว
            <Link href="/home/alldata" style={{ marginLeft:12, color:'#15803d', fontWeight:700 }}>← กลับหน้าหลัก</Link>
          </div>
        )}
        {status === 'error' && (
